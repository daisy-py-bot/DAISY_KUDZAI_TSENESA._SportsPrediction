...
Original file is located at
    https://colab.research.google.com/drive/1DcjtTzzSYzMGDFSMhK1bxHT-5uERgCba
...
def encode_categorical_data(data):
    ...
    # Convert encoded_data to DataFrame with proper column names
...
def model_training(x_train, x_test, y_train, y_test):
    ...
    for name, model in models.items():
        ...
        print(f"MAE: Mean = {mean_score:.4f}, Std = {scores.std():.4f}")  # Negate to show positive values for MAE
        print("-" * 30)


    # Identify the best model based on the mean MAE score
    best_model_name = min(model_scores, key=model_scores.get)  # For MAE, lower is better
    best_model_score = model_scores[best_model_name]

    print(f"Best Model: {best_model_name}")
    print(f"Best Model Score (MAE): {best_model_score:.4f}")  # Negate to show positive values for MAE
...
def extract_important_features(best_xgb_model):
  ...
  print(importance_df.head())  # Displaying the top 10 features
...
# make predictions and measure the performance of the model
...
def test_model(data):
  # separate the target from the features
  Y = data['overall'] # get the target
  X = data[feature_subset] #  extract the feautres
